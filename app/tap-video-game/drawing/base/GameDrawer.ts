...
export default abstract class BaseGameDrawer {
  ...
  public draw(game: TapVideoGame) {
    ...
    if (game.getStatus() === TapVideoGameStatus.Preparation) {
      ...
      this.preparingTimerDrawer.draw(Math.ceil(timeLeft), gameWindowWidth, gameWindowHeight);
    ...
    if (!game.getIsPersonInCameraBounds()) {
      this.incorrectPositionOverlayDrawer.draw(gameWindowWidth, gameWindowHeight);
    }
  }
}
